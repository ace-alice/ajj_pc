<template>
  <div :class="{ CommonBankCard: true }">
    <el-image class="bottom-image" style="height: 70px; width: 70px" :src="info['protocol_logo']" fit="cover">
      <template #placeholder>
        <div></div>
      </template>
      <template #error>
        <img style="height: 70px; width: 70px" :src="USDTDefaultImage" alt="" />
      </template>
    </el-image>
    ...
    <div class="card-name">
      <el-image class="tag-image" style="height: 16px; width: 16px" :src="info['protocol_logo']" fit="cover">
        <template #placeholder>
          <div></div>
        </template>
        <template #error>
          <img style="height: 16px; width: 16px" :src="USDTDefaultImage" alt="" />
        </template>
      </el-image>
      <div class="card-info">
        {{ info['protocol'] }}
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    return { USDTDefaultImage, toSetDefault, dealWithCardNumber };
  }
});
</script>
