<template>
  <div :class="{ DefaultBankCard: true }">
    <el-image class="icon-image" style="height: 88px; width: 88px" :src="info['protocol_logo']" fit="cover">
      <template #placeholder>
        <div></div>
      </template>
      <template #error>
        <img style="height: 88px; width: 88px" :src="USDTDefaultImage" alt="" />
      </template>
    </el-image>
    <div class="default-tag" v-if="!isWithdraw">当前默认</div>
    <div class="default-tag change" @click="changeBank" v-else>{{ list.length > 1 ? '更换' : '添加' }}</div>
    <div class="card-name">
      <el-image class="tag-image" style="height: 24px; width: 24px" :src="info['protocol_logo']" fit="cover">
        <template #placeholder>
          <div></div>
        </template>
        <template #error>
  ...
  setup(props, { emit }) {
    ...
    function changeBank() {
      if (list.value.length === 1) {
        ...
        return proxy.mittBus.emit('closeAddUsdtPopUps', true);
      }
      emit('show');
    }

    return { USDTDefaultImage, dealWithCardNumber, list, changeBank };
  }
});
</script>
