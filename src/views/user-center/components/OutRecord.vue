<template>
  <div class="OutRecord">
    ...
    >
        ...
        <template #default="props">
          <div class="expand-box">
            <div class="expand-box-column">
              <div>
                <span v-if="priceToFixed(props.row['draw_amount'])" :style="{ color: '#0EA94A' }"
                  >-{{ priceToFixed(props.row['draw_amount']) }}</span
                >
                <span v-else>--</span>
              </div>
              <div>
                交易状态:
                <span :style="{ color: setStatus(Number(props.row['draw_status_client']))[1] }">{{
                  setStatus(Number(props.row['draw_status_client']))[0]
                }}</span>
              </div>
              <div>
                交易时间:
              ...
              <div>
                交易方式:
                <span v-if="props.row['money_change_name']">{{ props.row['money_change_name'] }}</span>
                <span v-else>--</span>
              </div>
              <div class="order">
                订单编号:
                <span v-if="props.row['draw_order']"
                  >{{ props.row['draw_order'] }} <Copy :message="props.row['draw_order']"
                /></span>
                <span v-else>--</span>
              </div>
            </div>
      ...
      <el-table-column prop="money_change_name" label="类型" align="center" />
      ...
      <el-table-column prop="order_status_client" label="交易状态" align="center">
        <template #default="scope">
          <span :style="{ color: setStatus(Number(scope.row['draw_status_client']))[1] }">{{
            setStatus(Number(scope.row['draw_status_client']))[0]
          }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="recharge_amount" label="金额" align="center">
        <template #default="scope">
          <span>{{ priceToFixed(scope.row['draw_amount']) }}</span>
        </template>
      </el-table-column>
      <template #empty>
        <NoDataPage />
...
import { getWithdrawList } from '@/api/user-center/in-out-record';
...
export default defineComponent({
  ...
  setup(props) {
    const {
      ...
      loading,
      resetForm
    } = formDataHook(getWithdrawList, 10, false);

    watch(
      () => props.active,
      (newVal) => {
        if (newVal === 'out-record') {
          resetForm();
        }
      }
    );
